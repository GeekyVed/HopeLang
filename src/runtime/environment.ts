...
export function createGlobalEnv(printHandler?: (msg: string) => void) {
    ...
    // Native Functions
    const printFn = printHandler ? printHandler : (msg: string) => console.log(msg);

    env.declareVar("print", MK_NATIVE_FN((args, scope) => {
        const text = args.map(a =>
            a.type === "number" ? (a as any).value :
                a.type === "string" ? (a as any).value :
                    a.type === "boolean" ? (a as any).value :
                        a.type === "null" ? "null" :
                            a
        ).join(" ");
        printFn(text);
